class Solution {
public:
    cn.example.string numberToWords(int num) {
...
private:
    ...
    static cn.example.string f(int n){
        ...
        else if(n>=20) return cn.example.string(" ")+a_100[n/10-2]+f(n-10*(n/10));
        else if(n>=1) return cn.example.string(" ")+a_20[n-1];
        else return "";
    }
};
